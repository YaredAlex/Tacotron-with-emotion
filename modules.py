...
def get_encoder(hp):
    ...
    inputs2 = tf.keras.layers.Input(shape=(None,))
    e_embedding = tf.keras.layers.Embedding(5,hp.embed_size)(inputs2)
    
    ##### Embedding and prenet
    embedded_input, mask =  EmbeddingwithMask(hp)(inputs)
 
    prenet_out = Prenet(hp.encoder_prenet_size, hp.dropout_rate)(embedded_input+e_embedding)
    ...
    model = tf.keras.Model([inputs,inputs2], [memory, final_state, mask])
    return model
