...
def training(dataloader, hp):
    ...
    decoder1 = AttentionDecoder(attention_mechanism, hp)
    ##### Define decoder2 (for linear spectrogram)
    ...
    while 1:
        ...
        start = time.time()
        batch_index = 0
        #return values are text_decoded, mel, mag
        for x, y, z, e in tqdm(dataloader.loader):
            batch_index += 1
            ...
            with tf.GradientTape() as tape:
                ...
                memory, memory_state, memory_mask = encoder([x,e])
            ...
            for i, grads in enumerate(gradients):
                if isinstance(grads, tf.IndexedSlices):
                    ...
                    if len(grads.get_shape()) > 0:
                        grad_norm.append(np.linalg.norm(grads.numpy()))
            ...
            if step_index % 100 == 0:
                ...
                audio_data = spectrogram2wav(z_hat[0].numpy())
                output_file = 'output_step_index' + str(step_index) + '.wav'
                write(output_file, hp.sr, audio_data)
                # Training Summary
                with train_summary_writer.as_default():
                    ...
                    tf.summary.audio("Test Voice", tf.convert_to_tensor(spectrogram2wav(z_hat[0].numpy()).reshape((1, -1, 1)), dtype=tf.float32), sample_rate=hp.sr, step = step_index)
        ...
        print(f"Mel-Spectrogram Loss: {loss1.numpy()}")
        print(f"Linear Spectrogram Loss: {loss2.numpy()}")
        loss_recorder['loss1'].append(loss1.numpy())
        loss_recorder['loss2'].append(loss1.numpy())
...
if __name__ == "__main__":
    ...
    if hp.source == "ESD":
        ...
        hp.vocab = symbols
        dl = DataLoader(hp)
    ##### Choose type of attention mechanism; 
